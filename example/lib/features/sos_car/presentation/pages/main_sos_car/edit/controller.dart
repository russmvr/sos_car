...
class Controller extends XController<Event,State,Router, Interactor> {
  ...
  Stream<State> mapEvent(event, currentState) async* {
    ...
    if(event is GoToSelectionServicesPage){
      router.goToSelectionServicesPage(event.allInfo);
    }
    if(event is GoToEditCarInfo){
      router.goToEditCarInfo(event.info);
    }
    if(event is GoToEditEvacType){
      router.goToEditEvacType(event.info);
    }
  }
